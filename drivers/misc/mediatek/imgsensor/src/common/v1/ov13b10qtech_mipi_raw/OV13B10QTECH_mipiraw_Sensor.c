 ...
 * Setting version:
 * ------------
 *   update full pd setting for OV13B10QTECH
 *------------------------------------------------------------------------------
...
#include "OV13B10QTECH_mipiraw_Sensor.h"
...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data ={
	.sensorID= OV13B10QTECH_SENSOR_ID,
	.deviceID = 1,
	.dataLength = 0x1BF6,
...
#if 0
static struct stCAM_CAL_CHECKSUM_STRUCT ov13b10qtechChecksum[8] =
{
...
static void preview_setting(void)
{
	...
	ov13b10qtech_table_write_cmos_sensor(
		...
		sizeof(addr_data_pair_preview_ov13b10qtech) / sizeof(kal_uint16));
#else
//	write_cmos_sensor(0x0100, 0x00);
...
#define OTP_OV13B10QTECH 1
#define INCLUDE_NO_OTP_OV13B10QTECH 0
#if OTP_OV13B10QTECH
#define OV13B10QTECH_EEPROM_SLAVE_ADD 0xB0
#define OV13B10QTECH_SENSOR_IIC_SLAVE_ADD 0x6c
#define OV13B10QTECH_MODULE_ID  0x06

typedef struct ov13b10qtech_otp_data {
	unsigned short module_id;
} OV13B10QTECH_OTP_DATA;

OV13B10QTECH_OTP_DATA ov13b10qtech_otp_data;
...
static int  ov13b10qtech_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add,kal_uint8 size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		...
		LOG_INF("+++OV13B10QTECH otp module_id[%d] = 0x%x\n",i,module_id[i]);
		start_add ++;
	}
	ov13b10qtech_otp_data.module_id = module_id[0];
	LOG_ERR("ov13b10qtech_otp_data.module_id= 0x%x",ov13b10qtech_otp_data.module_id);

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = OV13B10QTECH_SENSOR_IIC_SLAVE_ADD;
	spin_unlock(&imgsensor_drv_lock);

	return ov13b10qtech_otp_data.module_id;
}
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_DBG("OV13B10QTECH_I2CBUS = %d, I2C_bus = %d\n", OV13B10QTECH_I2CBUS, I2C_bus);
	if (I2C_bus != OV13B10QTECH_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		LOG_ERR("OV13B10QTECH_I2CBUS: %d, I2C_bus = %d, Check Error!\n", OV13B10QTECH_I2CBUS, I2C_bus);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		i++;
#if 0
		size = imgSensorReadEepromData(&sensor_eeprom_data,ov13b10qtechChecksum);
...
#if OTP_OV13B10QTECH
				ov13b10qtech_read_data_from_eeprom(OV13B10QTECH_EEPROM_SLAVE_ADD,0x000B,1);
#if INCLUDE_NO_OTP_OV13B10QTECH
				if ((ov13b10qtech_otp_data.module_id > 0) && (ov13b10qtech_otp_data.module_id < 0xFFFF)) {
#endif
					if (ov13b10qtech_otp_data.module_id != OV13B10QTECH_MODULE_ID) {
						*sensor_id = 0xFFFFFFFF;
						return ERROR_SENSOR_CONNECT_FAIL;
					} else {
						LOG_INF("This is qtech --->ov13b10 otp data vaild ...");
						hq_regiser_hw_info(HWID_MAIN_CAM, "qtech ov13b10 13m");
					}
#if INCLUDE_NO_OTP_OV13B10QTECH
				} else {
		...
		LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n",
			imgsensor.i2c_write_id, sensor_id);
				...
				if ((ov13b10qtech_otp_data.module_id > 0) && (ov13b10qtech_otp_data.module_id < 0xFFFF)) {
					...
					LOG_INF("This is ov13b10qtech, but no otp data ...");
				}
#endif
#endif
		break;
