...
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_EMPTY: {
			...
			flag = gc5035_otp_read_byte(GC5035_OTP_MODULE_FLAG_OFFSET + GC5035_OTP_GET_OFFSET((idx +1) * (GC5035_OTP_MODULE_DATA_SIZE + 1)));
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			LOG_INF("group %d is valid!\n", idx + 1);
			module_start_offset = GC5035_OTP_MODULE_DATA_OFFSET
				+ GC5035_OTP_GET_OFFSET(idx * (GC5035_OTP_MODULE_DATA_SIZE + 1));
			...
			if ((check % 255 + 1) == info[GC5035_OTP_MODULE_DATA_SIZE - 1]) {
				...
				pWB->flag = GC5035_OTP_FLAG_VALID;
				...
				pGolden->flag = GC5035_OTP_FLAG_VALID;
...
#define OTP_GC5035 1
#define INCLUDE_NO_OTP_GC5035 1
#if OTP_GC5035
#define GC5035_OFILM_MODULE_ID  0x07
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				module_id = gc5035_otp_identify();
				pr_debug("[gc5035_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x, module_id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id, module_id);
#if OTP_GC5035
#if INCLUDE_NO_OTP_GC5035
				if ((module_id > 0) && (module_id < 0xFFFF)) {
#endif
					if (module_id != GC5035_OFILM_MODULE_ID) {
						*sensor_id = 0xFFFFFFFF;
						return ERROR_SENSOR_CONNECT_FAIL;
					} else {
						LOG_INF("This is ofilm --->gc5035 otp data vaild ...");
						hq_regiser_hw_info(HWID_SUB_CAM, "ofilm gc5035 5m");
					}
#if INCLUDE_NO_OTP_GC5035
				} else {
