...
static ssize_t ps_store_enable(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t count)
{
	struct alsps_context *cxt = NULL;
	int value = -1;
...
static ssize_t ps_show_enable(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", enable);
}
static ssize_t ps_show_25cm(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", psensor_25_offset);
}
static ssize_t ps_show_ct(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", psensor_ct);
}
static ssize_t ps_show_psdata(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	struct alsps_context *cxt = NULL;
	int value = -1;
...
DEVICE_ATTR(psenable, 0644, ps_show_enable, ps_store_enable);
DEVICE_ATTR(ps25cm, 0644, ps_show_25cm, NULL);
DEVICE_ATTR(psct, 0644, ps_show_ct, NULL);
DEVICE_ATTR(psdata, 0644, ps_show_psdata, NULL);
