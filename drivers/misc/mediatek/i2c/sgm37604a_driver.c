...
unsigned char backlight_led_reg = 0x06;
unsigned char backlight_current_reg = 0xeb;
...
void SGM37604A_set_backlight_reg_init(void)
{
	...
	ret_code = SGM37604A_smbus_write_byte(new_client, SGM37604A_CTL_BRIGHTNESS_LSB_REG, &data1);
	if (ret_code != 0) {
		BACKLIGHT_ERR("[Cust_SetBacklight] CTL_LED fail: %d\n", ret_code);
	}
	BACKLIGHT_LOG("[SGM37604A] set backlight led reg = 0x%x \n", data1);

	ret_code = SGM37604A_smbus_write_byte(new_client, SGM37604A_CTL_BRIGHTNESS_MSB_REG, &data2);
	if (ret_code != 0) {
