...
static int bq2560x_set_ivl(struct charger_device *chg_dev, u32 volt)
{
	...
	pr_err("ti bq25601 debug %s: vindpm volt = %d\n", __func__, bq->platform_data->usb.vlim);

	return bq2560x_set_input_volt_limit(bq, bq->platform_data->usb.vlim/1000);
...
static int bq2560x_enable_discharging(struct charger_device *chg_dev, bool en)
{
	int ret;
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
	u8 pg_stat = 0;
...
static int bq2560x_enable_powerpath(struct charger_device *chg_dev, bool en)
{
	...
	if (en)
		bq2560x_set_input_volt_limit(bq,4500);
	else
...
static struct charger_ops bq2560x_chg_ops = {
	...
	.is_otg = bq2560x_is_otg,
	.set_boost_current_limit = bq2560x_set_boost_ilmt,
	.enable_discharge = bq2560x_enable_discharging,
