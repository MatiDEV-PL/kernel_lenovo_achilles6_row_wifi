...
static struct proc_dir_entry *fts_gesture_ctr_proc;
...
static int fts_ito_test_show(struct seq_file *file, void* data)
{
    ...
    for( i = FT8201_ENTER_FACTORY_MODE ; i <= FT8201_SHORT_CIRCUIT_TEST;  i++ ){
    ...
    if((fts_ftest->test_item[FT8201_ENTER_FACTORY_MODE].testresult))
    {
    ...
    //Rawdata result
    if ((fts_ftest->test_item[FT8201_RAWDATA_TEST].testresult)
        &&(fts_ftest->test_item[FT8201_CB_TEST].testresult))
    {
    ...
    //open test result
    if (fts_ftest->test_item[FT8201_OPEN_TEST].testresult)
    {
    ...
    //short test result
    if(fts_ftest->test_item[FT8201_SHORT_CIRCUIT_TEST].testresult)
    {
...
static const struct file_operations fts_proc_Gesture_ops = {
	.owner = THIS_MODULE,
	.read = fts_GESTURE_read,
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    fts_gesture_ctr_proc = proc_create(FTS_GESTURE_CONTTROL, 0666,
    fts_android_touch_proc, &fts_proc_Gesture_ops);
    if (!fts_gesture_ctr_proc)
        FTS_TEST_ERROR("create proc entry %s failed",FTS_GESTURE_CONTTROL);
    else
        FTS_TEST_DBG("create proc entry %s success",FTS_GESTURE_CONTTROL);
