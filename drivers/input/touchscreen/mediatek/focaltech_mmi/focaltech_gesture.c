...
uint8_t FTS_PROC_SEND_FLAG;
ssize_t fts_GESTURE_read(struct file *file, char *buf,
		size_t len, loff_t *pos)
{
    ...
    if (!FTS_PROC_SEND_FLAG) {
        mutex_lock(&input_dev->mutex);
        temp_buf = kcalloc(len, sizeof(char), GFP_KERNEL);
        count = snprintf(temp_buf, PAGE_SIZE, "Gesture Mode:%s\n",
                         fts_gesture_data.mode ? "On" : "Off");
        ...
        kfree(temp_buf);
        mutex_unlock(&input_dev->mutex);
        FTS_PROC_SEND_FLAG = 1;
    } else {
    FTS_PROC_SEND_FLAG = 0;
    }

    return count;
}

ssize_t fts_GESTURE_write(struct file *file, const char *buff,
		size_t len, loff_t *pos)
{
    struct input_dev *input_dev = fts_data->input_dev;
	...
	if (len >= 80) {
		FTS_INFO("%s: no command exceeds 80 chars.\n", __func__);
		return -EFAULT;
	}
